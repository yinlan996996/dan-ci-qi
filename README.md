...
- 葫芦记忆法：按每日词数拆成用户自定义大小的分组，先记忆再测验，达到 80% 才能进入下一组。
...
1. 进入底部的“葫芦”页面。
2. 设置每天学习多少个词，默认 1000 个；设置目标轮数，默认 5 轮。
3. 设置“每组词数”，然后点“开始葫芦记忆法”。系统会按设置拆分计划，默认每组 20 个词。
4. 先查看当前组的英文和中文，记忆完成后点“记忆完成，开始测验”。
5. 系统会随机打乱这 20 个词，并用八选一进行测验，也可以点“我不会”。
6. 当前组需要达到 80% 才能进入下一组；例如每组 20 个词需要答对至少 16 个，每组 25 个词需要答对至少 20 个。不足 80% 会重新展示并测验同一组。
7. 每组和每轮都会自动保存。点“退出并保存”后，下次进入“葫芦”会继续原来的组和轮次。
8. 一轮完成后可以开始下一轮，下一轮仍然使用当天的同一批单词。

如果已经有计划后修改“每日词数”，再次点击开始时会提示创建新的计划；原来的错题记录仍会保留。记忆清单阶段和测验阶段都可以点击“退出并保存”。

每日词数最少为 20 个；每组词数可以设置为 1 到 100 个，但不会超过每日词数。修改每日词数或每组词数后，再次点击开始会创建新的葫芦计划，原来的错题记录仍会保留。

葫芦记忆法与普通“筛查”使用独立进度。导出“数据文件”时会同时保存两种模式的词库、错题和进度，可以在 iPhone、iPad 与电脑之间迁移。
