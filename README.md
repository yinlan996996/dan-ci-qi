...
- 葫芦记忆法：按每日词数拆成每组 20 词，先记忆再测验，达到 80% 才能进入下一组。
...
3. 点“开始葫芦记忆法”。系统会把计划拆成每组 20 个词。
4. 先查看当前组的英文和中文，记忆完成后点“记忆完成，开始测验”。
5. 系统会随机打乱这 20 个词，并用八选一进行测验，也可以点“我不会”。
6. 当前组需要答对至少 16 / 20 才能进入下一组；不足 80% 会重新展示并测验同一组。
...
每日词数最少为 20 个，因为每组固定 20 个；输入 2 会自动按 20 个处理。设为 20 时需要至少 16 / 20，想练两组可以设为 40。
